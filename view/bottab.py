...
class BotTab(Frame):
    def __init__(self, parent, presenter) -> None:
        ...
        self.time_label = Label(
            self.bot_frame, text="Time", font=('Arial', 12))
        ...
        self.symbol_label = Label(
            self.bot_frame, text="Symbol", font=('Arial', 12))
        ...
        self.amount_used = Label(
            self.bot_frame, text="percentage bot use", font=('Arial', 12))
        self.amount_slider = Scale(
            self.bot_frame, from_=0, to_=100, orient='horizontal')
        self.amount_slider.config(value=50)

        self.profit_used = Label(
            self.bot_frame, text="profit percentage", font=('Arial', 12))
        self.profit_slider = Scale(
            self.bot_frame, from_=0, to_=100, orient='horizontal')
        self.profit_slider.config(value=15)

        self.loss_used = Label(
            self.bot_frame, text="loss percentage", font=('Arial', 12))
        self.loss_slider = Scale(
            self.bot_frame, from_=0, to_=100, orient='horizontal')
        ...
        self.start_button = Button(
            self.bot_frame, text="Start Bot", command=self.start_bot)
        self.start_button.config(width=15)

        self.stop_button = Button(
            self.bot_frame, text="Stop Bot", command=self.stop_bot)
        self.stop_button.config(width=15)

        self.create_button = Button(
            self.bot_frame, text="Create Bot", command=self.create_bot)
        self.create_button.config(width=15)

        self.destroy_button = Button(
            self.bot_frame, text="Destroy Bot", command=self.destroy_bot)
        self.destroy_button.config(width=15)

        # AUTO TRADING BOT

        self.options_ml_frame.grid(row=0, column=0, padx=5, pady=5)
        self.options_ml_model.grid(
    ...
    def add_bot_to_optionmenu(self, count: int, name) -> None:
        self.bot_select["menu"].add_command(
            label=f"{name}{count}", command=lambda: self.bot_select_var.set(f"{count}"))
    ...
    def update_bot_status(self, status: str, index: int, name) -> None:
        self.bot_select["menu"].entryconfigure(
            index, label=f"{name}{index + 1} ({status})")
