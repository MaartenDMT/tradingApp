...
def generate_random_candlestick_data(
        num_candles: int, initial_price: float = 100, min_volatility: float = 0.01, max_volatility: float = 0.05,
        max_shadow_amplitude: float = 0.5, max_volume_multiplier: float = 5.0, start_date: datetime = None,
        seed: int = None, bias_factor: float = 0.001) -> tuple:
    """
    ...
    Args:
        ...
        initial_price (float, optional): Initial price. Defaults to 100.
        ...
        bias_factor (float, optional): Bias factor to adjust the mean of the price movement distribution. Defaults to 0.001.
    ...
    volatilities = np.random.uniform(
        min_volatility, max_volatility, num_candles)

    for i in range(num_candles):
        open_prices[i] = initial_price
        price_movement = np.random.normal(
            bias_factor * initial_price, volatilities[i] * initial_price)
        # Ensure positive prices
        close_prices[i] = max(0.01, open_prices[i] + price_movement)

        shadow_amplitude = np.random.uniform(
            0, max_shadow_amplitude) * abs(price_movement)
        high_prices[i] = max(
            open_prices[i], close_prices[i]) + shadow_amplitude
        ...
        volume[i] = max(0, base_volume *
                        np.random.uniform(1.0, max_volume_multiplier))
...
def plot_candlestick_chart(candlestick_data, start_date, title='Candlestick Chart'):
    ...
    candlestick_data = pd.DataFrame(candlestick_data, columns=['open', 'high', 'low', 'close', 'volume'], index=pd.date_range(
        start_date, periods=len(candlestick_data), freq="30min"))
    mpf.plot(candlestick_data, type='candle', mav=(3, 6, 9),
             volume=True, show_nontrading=True, style='yahoo', title=title)
