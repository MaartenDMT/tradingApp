...
# Conditional imports to avoid TensorFlow/PyTorch conflicts
try:
    from stable_baselines3 import A2C, DQN, PPO, TD3
    STABLE_BASELINES3_AVAILABLE = True
except ImportError as e:
    print(f"Warning: stable_baselines3 import failed: {e}")
    print("Stable Baselines3 models will not be available.")
...
# Tensorflow models - with error handling
try:
    import model.reinforcement.algorithms.tensorflow.TF.AC.ac_tf as AC_tf
    import model.reinforcement.algorithms.tensorflow.TF.DDQN.ddqn_tf as DDQN_tf
    import model.reinforcement.algorithms.tensorflow.TF.MAPDDG.mapddg_tf as MAPDDG_tf
    import model.reinforcement.algorithms.tensorflow.TF.PG.pg_tf as PG_tf
    import model.reinforcement.algorithms.tensorflow.TF.PPO.ppo_tf as PPO_tf
    import model.reinforcement.algorithms.tensorflow.TF.SAC.sac_tf as SAC_tf
    TENSORFLOW_MODELS_AVAILABLE = True
except ImportError as e:
    print(f"Warning: TensorFlow models import failed: {e}")
...
from model.reinforcement.utils.rl_utilities import \
    generate_random_candlestick_data
from model.reinforcement.utils.visualization.rl_visual import \
    plot_and_save_metrics
from model.reinforcement.utils.visualization.visual_plot import (
...
class TensorflowModel:
    def __init__(self, params, rl_logger, num_agents=1) -> None:
        ...
        self.env = TradingEnvironment(symbol='BTCUSDT', features=params['features'],
...
class StablebaselineModel:
    def __init__(self, params, rl_logger, num_agents=1):
        if not STABLE_BASELINES3_AVAILABLE:
            raise ImportError("Stable Baselines3 is not available. Please check your installation.")
