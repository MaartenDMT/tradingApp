...
async def bot():
    ...
    for symbol in symbols:
        ...
        symbol_prices = [exchange_prices[symbol]['last']
        ...
        if (profit > 0):  # not taking into account slippage or order book depth
            print(ms, symbol, "profit:", profit, "Buy", min_exchange.id,
                  min_price, "Sell", max_exchange.id, max_price)

            if not paper_trading:
                buy_min = min_exchange.create_market_buy_order(
                    symbol, order_size)
                sell_max = max_exchange.create_market_sell_order(
                    symbol, order_size)
                # execute them "simultaneously"
                orders = await asyncio.gather(buy_min, sell_max)
