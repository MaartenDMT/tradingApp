...
# from model.reinforcement.agents.legacy.agent_manager import StablebaselineModel
...
try:
    ...
    loggers.setup_loggers()['model'].warning("Optimized trading system not available")
...
try:
    ...
    loggers.setup_loggers()['model'].warning("ML system not available")
...
try:
    ...
    loggers.setup_loggers()['model'].warning("RL system not available")
...
class Models:
    ...
    def create_tabmodels(self, presenter) -> None:
        ...
        self._trading_system_model = None
        self._ml_system_model = None
        self._rl_system_model = None
    ...
    # New optimized systems initialization methods
    def _ensure_trading_system_model(self):
        if self._trading_system_model is None and TRADING_SYSTEM_AVAILABLE:
            self.model_logger.debug("Initializing TradingSystemModel")
            self._trading_system_model = TradingSystemModel(self._presenter)
    
    def _ensure_ml_system_model(self):
        if self._ml_system_model is None and ML_SYSTEM_AVAILABLE:
            self.model_logger.debug("Initializing MLSystemModel")
            self._ml_system_model = MLSystemModel(self._presenter)
    
    def _ensure_rl_system_model(self):
        if self._rl_system_model is None and RL_SYSTEM_AVAILABLE:
            self.model_logger.debug("Initializing RLSystemModel")
            self._rl_system_model = RLSystemModel(self._presenter)
    ...
    def trading_system_model(self):
        self._ensure_trading_system_model()
        return self._trading_system_model
    
    @property
    def ml_system_model(self):
        self._ensure_ml_system_model()
        return self._ml_system_model
    
    @property
    def rl_system_model(self):
        self._ensure_rl_system_model()
        return self._rl_system_model
...
class BotTabModel:
    ...
    def get_data_ml_files(self) -> list:
        try:
            ...
            import asyncio
            cache = asyncio.run(get_global_cache())
            cached_files = asyncio.run(cache.get("ml_files_list"))
            ...
            if os.path.exists(path):
                ...
                asyncio.run(cache.set("ml_files_list", files, memory_ttl=60, redis_ttl=60))
                return files
            else:
...
# New Optimized System Models

class TradingSystemModel:
    """Model for the optimized trading system."""
    
    def __init__(self, presenter) -> None:
        self.model_logger = logger['model']
        self.presenter = presenter
        self.model_logger.info("Loading the Trading System model")
        
        self._trading_system = None
        self._trading_config = None
        
    def get_trading_system(self, config_dict=None):
        """Get or create the trading system instance."""
        ...
        if self._trading_system is None:
            ...
            self.model_logger.info("Trading system created successfully")
    ...
    def get_system_status(self):
        ...
        if self._trading_system:
            return self._trading_system.get_system_status()
...
class MLSystemModel:
    """Model for the ML system integration."""
    
    def __init__(self, presenter) -> None:
        self.model_logger = logger['model']
        self.presenter = presenter
        self.model_logger.info("Loading the ML System model")
        
        self._ml_systems = {}  # Dictionary to hold different ML systems by name
    
    def create_ml_system(self, name, algorithm='random_forest', target_type='classification', config_dict=None):
        """Create a new ML system instance."""
        if not ML_SYSTEM_AVAILABLE:
            self.model_logger.error("ML system is not available")
            return None
        ...
        try:
            ...
            ml_config = MLConfig(
                ...
                hyperparameter_optimization=config_dict.get('hyperparameter_optimization', True) if config_dict else True,
                random_state=42
            )
            ...
            # Create ML system
            ml_system = MLSystem(ml_config)
            self._ml_systems[name] = {
                'system': ml_system,
                'config': ml_config,
                'created_at': pd.Timestamp.now()
            }
            
            self.model_logger.info(f"ML system '{name}' created with algorithm: {algorithm}")
            return ml_system
            
        except Exception as e:
            self.model_logger.error(f"Error creating ML system '{name}': {e}")
            return None
    
    def get_ml_system(self, name):
        """Get an existing ML system by name."""
        return self._ml_systems.get(name, {}).get('system')
    ...
    def list_ml_systems(self):
        ...
        return {
            name: {
                ...
                'target_type': info['config'].target_type,
                'created_at': info['created_at'],
                'is_fitted': info['system'].is_fitted
            }
            for name, info in self._ml_systems.items()
        }
    
    def train_ml_system(self, name, X_train, y_train, **kwargs):
        """Train a specific ML system."""
        ml_system = self.get_ml_system(name)
        if not ml_system:
            return {'success': False, 'message': f'ML system {name} not found'}
        
        try:
            results = ml_system.train(X_train, y_train, **kwargs)
            self.model_logger.info(f"ML system '{name}' trained successfully")
            return {'success': True, 'results': results}
        except Exception as e:
            self.model_logger.error(f"Error training ML system '{name}': {e}")
            return {'success': False, 'message': str(e)}
    ...
    def predict_with_ml_system(self, name, X):
        ...
        try:
            predictions = ml_system.predict(X)
            return {'success': True, 'predictions': predictions.tolist()}
        except Exception as e:
            self.model_logger.error(f"Error making predictions with ML system '{name}': {e}")
            return {'success': False, 'message': str(e)}


class RLSystemModel:
    """Model for the RL system integration."""
    
    def __init__(self, presenter) -> None:
        self.model_logger = logger['model']
        self.presenter = presenter
        self.model_logger.info("Loading the RL System model")
        
        self._rl_system_manager = None
        self._agents = {}  # Dictionary to hold different RL agents by name
    
    def get_rl_system_manager(self):
        """Get or create the RL system manager."""
        if not RL_SYSTEM_AVAILABLE:
            self.model_logger.error("RL system is not available")
            return None
            
        if self._rl_system_manager is None:
            self._rl_system_manager = RLSystemManager()
            self.model_logger.info("RL system manager created")
        
        return self._rl_system_manager
    
    def create_agent(self, name, agent_type, state_dim, action_dim, config=None):
        """Create a new RL agent."""
        rl_manager = self.get_rl_system_manager()
        if not rl_manager:
            return None
        
        try:
            agent = rl_manager.create_agent(agent_type, state_dim, action_dim, config)
            self._agents[name] = {
                'agent': agent,
            ...
            self.model_logger.info(f"RL agent '{name}' created with type: {agent_type}")
            return agent
            
        except Exception as e:
            self.model_logger.error(f"Error creating RL agent '{name}': {e}")
            return None
    
    def get_agent(self, name):
        """Get an existing RL agent by name."""
        return self._agents.get(name, {}).get('agent')
    ...
    def train_agent(self, name, environment_data, config=None):
        """Train a specific RL agent."""
        agent = self.get_agent(name)
        rl_manager = self.get_rl_system_manager()
        
        if not agent or not rl_manager:
            return {'success': False, 'message': f'Agent {name} or RL manager not found'}
        
        try:
            # Create environment from data
            environment = rl_manager.create_environment(environment_data, config)
            
            # Train the agent
            results = rl_manager.train_agent(agent, environment, config)
            
            self.model_logger.info(f"RL agent '{name}' trained successfully")
            return {'success': True, 'results': results}
            
        except Exception as e:
            self.model_logger.error(f"Error training RL agent '{name}': {e}")
            return {'success': False, 'message': str(e)}
    
    def evaluate_agent(self, name, environment_data, num_episodes=10):
        ...
        try:
            # Create environment from data
            environment = rl_manager.create_environment(environment_data)
            
            # Evaluate the agent
            results = rl_manager.evaluate_agent(agent, environment, num_episodes)
            
            return {'success': True, 'results': results}
            
        except Exception as e:
            self.model_logger.error(f"Error evaluating RL agent '{name}': {e}")
            return {'success': False, 'message': str(e)}
    ...
    def get_available_algorithms(self):
        ...
        rl_manager = self.get_rl_system_manager()
        if rl_manager:
            return rl_manager.get_available_agents()
        return {}
