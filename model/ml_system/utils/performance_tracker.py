...
class PerformanceTracker:
    ...
    def get_performance_summary(self, experiment_name: str = None) -> Dict[str, Any]:
        ...
        if experiment_name and experiment_name in self.experiment_data:
            return {
                'metrics': self.experiment_data[experiment_name]['metrics'],
                'experiment_count': len(self.experiment_data[experiment_name]['metrics'])
            }
    ...
    def export_data(self, experiment_name: str = None):
        """
        Export performance data for external analysis.

        Args:
            experiment_name: Optional experiment name to filter data

        Returns:
            List of performance metrics or all data if no experiment specified
        """
        if experiment_name and experiment_name in self.experiment_data:
            return self.experiment_data[experiment_name]['metrics']

        # Return all data if no specific experiment
        return {
